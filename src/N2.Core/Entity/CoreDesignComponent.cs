...
#pragma warning disable CA1031 // Do not catch general exception types
        using TContext Db = await DbFactory.CreateAsync();
        IQueryable<TData> recordSet = RecordSet.Invoke(Db);
        try
        {
            TData? record;
            if (model.DbItem)
            {
                ...
                record.UpdateTracking(userContext.PublicId);
                record = await MapViewToRecord.Invoke(model, record);

                Db.AddChangeLog<TData>(record.PublicId, "Modified", userContext.PublicId, userContext.Name);
            }
            else if (CanDesign(userContext, model.PublicId))
            {
                ...
                record.TrackingSetCreated(userContext.PublicId);
                record = await MapViewToRecord.Invoke(model, record);
                Db.AddRecord(record);
                Db.AddChangeLog<TData>(record.PublicId, "Added", userContext.PublicId, userContext.Name);
            }
            else
            {
                return new RequestResult(403, "No design rights");
            ...
            (Commands.ResponseStatus result, string message) = await Db.SaveChangesAsync();
...
#pragma warning restore CA1031 // Do not catch general exception types
    ...
    public async Task<RequestResult> RemoveItemAsync(Guid publicId, IUserContext? userContext)
    {
        ...
        using TContext Db = await DbFactory.CreateAsync();
        (int resultcode, string message) = await Db.DeleteAsync<TData>(publicId);
        if (resultcode.IsErrorCode())
        {
            return new RequestResult(resultcode, message);
        }

        Db.AddChangeLog<TData>(publicId, "Removed", userContext.PublicId, userContext.Name);

        (Commands.ResponseStatus status, string saveMessage) = await Db.SaveChangesAsync();
        return new RequestResult(status, saveMessage);
    }
    ...
    public async Task<TDetail?> InitializeModelAsync(string? id, IUserContext? userContext)
    {
        ...
        TDetail? result;
        if (Guid.TryParse(id, out Guid publicId))
        {
        ...
        result.InitTracking(publicId, userContext.PublicId);
        return result;
    }
    ...
    public async Task<bool> LoadItemsAsync([NotNull] PagingInfo pagingInfo, IUserContext? userContext)
    {
        ...
        using TContext Db = await DbFactory.CreateAsync();
        IQueryable<TData> recordSet = RecordSet
            .Invoke(Db)
            .Where(m => !m.IsRemoved);
        (IEnumerable<TData> items, int count) = await recordSet
            .SearchAsync(
                pagingInfo.Q,
                pagingInfo.Page,
                pagingInfo.PageSize,
        ...
        int totalPages = (int)Math.Ceiling(count / (double)pagingInfo.PageSize);
        int currentPage = pagingInfo.Page;
    ...
    public async Task<TDetail?> ReadFromDatabaseAsync(Guid publicId, IUserContext? userContext)
    {
        ...
        TData? record = await recordSet.FirstOrDefaultAsync(x => x.PublicId == publicId && !x.IsRemoved);
        if (record == null)
        {
            return default;
        }
